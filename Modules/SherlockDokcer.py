...
class MamaYaDokserSherlack(loader.Module):
    ...
    async def client_ready(self, client, db):
        ...
        self.db = db
        self.daily_limits = {}
        self.phone_regex = re.compile(r"(\+?\d{1,3}[-.\s]?\(?\d{3}\)?[-.\s]?\d{3}[-.\s]?\d{4})|(\+?\d{10,15})")
    ...
    async def doxcmd(self, message):
        ...
        if self.daily_limits.get(user_id, {}).get(today, 0) >= 1:
        ...
        bots = [
            ...
            "@SangMataInfo_bot",
            "@creationdatebot",
            "@SherlockBot",              # базовый шерлок, как ты просил
            "@funstat_searchings_bot"    # новый, как сказал
        ]
        ...
        for bot in bots:
            try:
                ...
                await asyncio.sleep(7)  # чуть больше ждём, чтоб все боты ответили
                history = await self.client.get_messages(bot, limit=20)
                for msg in history:
                    if msg.message:
                        text = msg.message
        ...
        phones_list = "\n".join(sorted(found_phones)) if found_phones else "Не найдено"
